...
function populateFormFields(object, prefix) {
    ...
    $.each(object, function (index, value) {
        ...
        var sam = $('#' + element).is("input,textarea");
        //TODO Refactor these multiple conditions
        if(sam === false) {
            sam = $('.' + element).is("input, textarea");
        }
        var ch = $('#' + element).attr('type');
        if(typeof ch === 'undefined') {
            ch = $('.' + element).attr('type');
        }

        if (sam) {
            switch (ch) {
                case 'radio': //missing radio, autocomplete, multiselect
        ...
        if ($('#' + element).is("select")) {
            ...
            if (jQuery.type(value) === 'string')
                value = value.split(',');
            //Event trigger help to update the select2 fields

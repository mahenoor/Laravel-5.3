                ...
                <div class="row {{ $det['is_mandatory'] ? "bg-info" : ""}}">
                    ...
                    <div class="col-md-3">
                        <div class="select2-selection__rendered">
                            {!! Form::select('expectation_'.$det['metrics_id'], $expectations , $selectedExpectationId , array('class' => 'form-control')) !!}
                        </div>
                    </div>
                </div>
            </section>
...
<div class="centered">
    ...
    @else
    <button type="button" class="btn btn-default" onclick="window.location='{{ route("adminReport") }}'">Cancel</button>
    @endif
</div>